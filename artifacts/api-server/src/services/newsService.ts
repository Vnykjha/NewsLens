...
export async function revalidateCache(apiKey: string): Promise<void> {
  ...
  try {
    ...
    // Fetch topics in parallel but staggered by 1 second to respect 2 req/sec rate limit
    await Promise.all(
      topics.map(async (topic, index) => {
        ...
        try {
          ...
          valid.forEach((a: any, idx: number) => {
            ...
            // Check for duplicate articles across categories to prevent duplicate React keys
